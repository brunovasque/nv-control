...
(function () {
  ...
  btn.addEventListener("click", async (e) => {
    ...
    try {
      ...
      // ===============================
      // FIX CANÔNICO — salvar plano aprovado
      // ===============================
      window.__APPROVED_BROWSER_PLAN__ = plan;
      console.log(
        "[BROWSER-UI] Plano aprovado salvo em window.__APPROVED_BROWSER_PLAN__"
      );

      // ===============================
      // FIX DO BUG ATUAL — definir "res"
      // ===============================
      const res = await fetch("https://run.nv-imoveis.com/browser/run", {
        method: "POST",
